...
export const EventList = ({ selectedCategory }: EventListProps) => {
  ...
  const filteredEvents = events.filter(event => {
    ...
    if (selectedCategory === 'panthers') {
      if (!event.title.toLowerCase().includes('panthers')) return false
    } else if (selectedCategory === 'hospitality') {
      if (!event.title.toLowerCase().includes('hospitality')) return false
    } else if (selectedCategory === 'all') {
      if (localCategory !== 'all' && !event.category.includes(localCategory)) return false
    }
